...
export const BAC_D_PROBABILITES: CurriculumSection = {
          ...
          content: {
            enonce_complet: `**EXERCICE TYPE BAC - PROBABILITÉS COMPLÈTES (NIVEAU LITTÉRAIRE)**

On considère une urne contenant 4 boules blanches (B) et 2 boules noires (N). On effectue plusieurs expériences aléatoires avec cette urne.
...
3. Calculer le nombre de codes à 2 chiffres possibles avec les boules (p-listes).
4. Calculer le nombre de façons d'élire un bureau de 2 personnes parmi 6 (arrangements).
5. Calculer le nombre de façons de classer 4 boules (permutations).
6. Calculer le nombre de façons de choisir 2 boules parmi 6 (combinaisons).
7. Utiliser les propriétés des combinaisons pour simplifier des calculs.
8. Construire le triangle de Pascal jusqu'à n=4.
9. Développer (1+1)^4 avec le binôme de Newton.
...
4. Adapter la difficulté au niveau littéraire (calculs plus simples)`,
            modules: {
              module_0: {
          ...
          // Niveau adapté au Bac D (littéraire) : calculs plus simples (4 blanches + 2 noires au lieu de 5+3)
          exercises: [],
          difficulty: 'Moyen',
          estimatedTime: 35
        }
