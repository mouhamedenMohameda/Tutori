...
export const BAC_C_LIMITES_DERIVEES_PRIMITIVES: CurriculumSection = {
  id: 'bac-2023-limites-derivees-primitives',
  title: 'Limites, continuité, dérivées et primitives',
  description: 'Limites, continuité, dérivées et primitives complètes: rappels sur limites, calcul de limites, asymptotes, limites composées, continuité, localisation de solutions, dérivabilité, calcul de dérivées, primitives usuelles, intégration. Modules progressifs couvrant tout le programme.',
  concepts: [
    // Limites
...
On considère une fonction f et plusieurs problèmes d'analyse.

**PARTIE I - LIMITES (Modules 1-4)**
1. Rappels: limites usuelles et lecture géométrique (encadrements, changements de variable, développements limités).
2. Opérations sur limites + formes indéterminées (divisions euclidiennes, complétion de carré, équivalents).
3. Asymptotes obliques + position relative (méthode unique: f, f/x, conclusion).
4. Limites composées + limites à gauche/droite (changements de variable, sin u/u → 1).

**PARTIE II - CONTINUITÉ ET ÉTUDE DE FONCTIONS (Modules 5-6)**
5. Continuité, prolongement, localisation de solution (TVI, dichotomie, vérification de continuité).
6. Étude de fonctions: domaine, parité, périodicité (plan d'étude standard).

**PARTIE III - DÉRIVABILITÉ ET VARIATIONS (Modules 7-9)**
7. Symétries (axe/centre) et transformations (plan d'étude complet).
8. Dérivée et variations (division euclidienne, factorisation, tableaux de signes, extremums).
9. Fonction réciproque (existence, graphique, dérivée, exemples: arcsin, fonctions homographiques).
...
5. Insister sur les techniques de calcul (limites, dérivées, primitives)
6. Faire pratiquer les méthodes standards issues des exercices corrigés
7. Illustrer géométriquement chaque notion
8. Utiliser les techniques des exercices corrigés: encadrements, changements de variable, développements limités, divisions euclidiennes, complétion de carré, équivalents
9. Générer des exercices TRÈS DIFFICILES comme dans les exemples corrigés fournis
10. Intégrer toutes les idées des exercices corrigés dans la génération des questions`,
    modules: {
      module_1: {
        ...
        contenu: {
          ...
          techniques_exercices: [
            ...
            'Changement de variable: u = π/2 - x pour transformer',
            'Développements limités: √(1+x) = 1 + x/2 - x²/8 + o(x²)',
            'Factorisation: (50+x²)² - 2500 = 100x² + x⁴',
            'Somme géométrique: 1+x+...+xⁿ = (1-xⁿ⁺¹)/(1-x)'
          ],
          illustration: 'Exemples: f(x) = xcos x/(1+x²), f(x) = (π/2-x)tan x, f(x) = (50+x²)²-2500/x²'
        }
      },
      module_2: {
        ...
        contenu: {
          ...
          techniques_standard: [
            ...
            'Remplacer par un équivalent / développement limité quand c\'est attendu (ex: √(1+x))'
          ],
          methodes_exercices: [
            'Division euclidienne pour fractions rationnelles: f(x) = ax+b + c/(x-d)',
            'Compléter le carré pour racines: x²-3x+1 = (x-3/2)² - 5/8',
            'Équivalents à l\'infini: √(x²+ax+b) ~ |x| pour x → ±∞',
            'Développements limités: √(1+x) = 1 + x/2 - x²/8 + o(x²)',
            'Somme géométrique: 1+x+...+xⁿ = (1-xⁿ⁺¹)/(1-x)'
          ],
          illustration: 'Exemples: f(x) = (-x²+x+1)/(x+2), f(x) = √(x²-3x+1), f(x) = (x⁵+...+1)/(x⁴-1)'
        }
      },
      module_3: {
        ...
        contenu: {
          ...
          position_relative: [
            ...
            'Exemple: f(x) = (-x²+x+1)/(x+2) = -x+3 - 5/(x+2), position selon signe de -5/(x+2)'
          ],
          ...
          exemples_rapides: [
            'f(x) = x³+x: f → +∞, f/x = x²+1 → +∞ ⇒ direction (Oy)',
            'f(x) = 3x-3√x: f/x = 3-3/√x → 3, mais f-3x = -3√x → -∞ ⇒ pas d\'asymptote affine',
            'f(x) = √(x²+2): f/x → 1, f-x → 0 ⇒ asymptote y = x'
          ],
          illustration: 'Exemples: fractions rationnelles, racines, fonctions avec √x'
        }
      },
      module_4: {
        ...
        contenu: {
          ...
          limites_unilaterales: [
            ...
            'Exemple: f(x) = (π/2-x)tan x, poser u = π/2-x, alors u → 0⁺ et tan x = cot u = cos u/sin u'
          ],
          techniques_exercices: [
            'Changement de variable: u = π/2 - x pour transformer tan(π/2-u) = cot u',
            'Utiliser sin u/u → 1 et cos u → 1',
            'Vérifier continuité de la fonction composée'
          ],
          illustration: 'Exemples: f(x) = (π/2-x)tan x, fonctions avec changement de variable'
        }
      },
      module_5: {
        ...
        contenu: {
          ...
          illustration: 'Exemples: fonctions avec point de discontinuité, utilisation du TVI'
        }
      },
      module_6: {
        ...
        contenu: {
          ...
          techniques_exercices: [
            'f(x) = 7x²+20x/(x²+2x-3): domaine ℝ∖{-3,1}',
            'f(x) = cos(3x)cos³(x): paire, période 2π',
            'f(x) = x√((x-1)/(x+1)): domaine (-∞,-1)∪[1,+∞)'
          ],
          illustration: 'Exemples: fonctions rationnelles, trigonométriques, avec racines'
        }
      },
      module_7: {
        ...
        contenu: {
          ...
          illustration: 'Exemples: fonctions paires/impaires, avec axes/centres de symétrie'
        }
      },
      module_8: {
        ...
        contenu: {
          ...
          techniques_exercices: [
            'f(x) = 2x²-7x+5/(x-3): division euclidienne, f\'(x) = 2 - 2/(x-3)²',
            'f(x) = x√((x-1)/(x+1)): dérivée avec chaîne et quotient',
            'f(x) = cos(3x)cos³(x): dérivée avec produit et chaîne',
          ...
          illustration: 'Exemples complets: fractions rationnelles, fonctions avec racines, trigonométriques'
        }
      },
      module_9: {
        ...
        contenu: {
          ...
          techniques_exercices: [
            'f(x) = (3x+1)/(x+2): trouver f⁻¹(y) = (2y-1)/(3-y)',
            'f(x) = sin x sur [-π/2, π/2]: f⁻¹ = arcsin',
            'Utiliser la symétrie pour tracer la courbe de f⁻¹'
          ],
          illustration: 'Exemples: fonctions rationnelles, trigonométriques, exponentielles/logarithmes'
        }
      },
      module_10: {
        ...
        contenu: {
          ...
          illustration: 'Exemples de calculs de primitives simples'
        }
      },
      module_11: {
        ...
        contenu: {
          ...
          illustration: 'Exemples de techniques d\'intégration'
        }
      }
    }
  },
